...
#        maxR = 1
        ...
        maxL = height[0] # initially max L is 0th index
        maxR = height[-1] # initially max R is last index

        totalArea = 0
        
        # We just compare maxes, the lower max becomes the bottleneck for area and
        # hence that guarentees that the area for that index can be found (as min of max decided it)
        # So we increment that side pointer and find area for new index
        # calculate the area and update the max for that side using the new index
