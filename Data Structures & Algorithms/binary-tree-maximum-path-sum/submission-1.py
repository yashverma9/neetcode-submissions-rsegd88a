...
class Solution:
    def maxPathSum(self, root: Optional[TreeNode]) -> int:
        ...
        maxVal = [root.val] # We use the 0th index of the list to store max value, as mutation is allowed in nested function not reassignments
        ...
        def findSum(node):
            ...
            maxSplitSum = max(node.val, node.val + leftSum + rightSum)
            
            if maxSplitSum > maxVal[0]:
                maxVal[0] = maxSplitSum
