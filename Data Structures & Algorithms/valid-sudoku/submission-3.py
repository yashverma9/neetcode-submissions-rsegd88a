class Solution:
    def isValidSudoku(self, board: List[List[str]]) -> bool:
        ...
        '''
            Intuition:
                Rule: When items are grouped into fixed sizes, use
                index // group_size to know which group an item belongs to.
                Example: In a cinema with 3 seats per row, seat_number // 3 gives the row.
                Sudoku: Each box is size 3, so row // 3 and col // 3 give the box.
        '''
        ...
        # Note- Instead of this logic, we can simply assume each box as a row, col
        # index of a new 3x3 box. So we can make the key as tuple of row, col pairs
        # So the indeces will be like subBoxHash[(row//3, col//3)]
